...
import { CORP_MOCK_API_DEFAULT_DELAY } from 'corp/lib/mock-api/mock-api.constants';
import { CorpMockApiService } from 'corp/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(CORP_MOCK_API_DEFAULT_DELAY);
    const corpMockApiService = inject(CorpMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = corpMockApiService.findHandler(request.method.toUpperCase(), request.url);
