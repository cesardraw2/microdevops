...
import { CorpNavigationItem, CorpVerticalNavigationAppearance, CorpVerticalNavigationMode, CorpVerticalNavigationPosition } from 'corp/components/navigation/navigation.types';
import { CorpVerticalNavigationAsideItemComponent } from 'corp/components/navigation/vertical/components/aside/aside.component';
...
import { CorpScrollbarDirective } from 'corp/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector       : 'corp-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : corpAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'corpVerticalNavigation',
    standalone     : true,
    imports        : [CorpScrollbarDirective, NgFor, NgIf, CorpVerticalNavigationAsideItemComponent, CorpVerticalNavigationBasicItemComponent, CorpVerticalNavigationCollapsableItemComponent, CorpVerticalNavigationDividerItemComponent, CorpVerticalNavigationGroupItemComponent, CorpVerticalNavigationSpacerItemComponent],
})
export class CorpVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: CorpVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: CorpVerticalNavigationMode = 'side';
    @Input() name: string = this._corpUtilsService.randomId();
    @Input() navigation: CorpNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: CorpVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<CorpVerticalNavigationAppearance> = new EventEmitter<CorpVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<CorpVerticalNavigationMode> = new EventEmitter<CorpVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<CorpVerticalNavigationPosition> = new EventEmitter<CorpVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<CorpNavigationItem> = new ReplaySubject<CorpNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<CorpNavigationItem> = new ReplaySubject<CorpNavigationItem>(1);
    ...
    private _corpScrollbarDirectives!: QueryList<CorpScrollbarDirective>;
    private _corpScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'corp-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`corp-vertical-navigation-appearance-${this.appearance}`]: true,
            'corp-vertical-navigation-hover'                          : this._hovered,
            'corp-vertical-navigation-inner'                          : this.inner,
            'corp-vertical-navigation-mode-over'                      : this.mode === 'over',
            'corp-vertical-navigation-mode-side'                      : this.mode === 'side',
            'corp-vertical-navigation-opened'                         : this.opened,
            'corp-vertical-navigation-position-left'                  : this.position === 'left',
            'corp-vertical-navigation-position-right'                 : this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for corpScrollbarDirectives
     */
    @ViewChildren(CorpScrollbarDirective)
    set corpScrollbarDirectives(corpScrollbarDirectives: QueryList<CorpScrollbarDirective>)
    {
        // Store the directives
        this._corpScrollbarDirectives = corpScrollbarDirectives;

        // Return if there are no directives
        if ( corpScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._corpScrollbarDirectivesSubscription )
        {
            this._corpScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._corpScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() =>
                {
                    ...
                    corpScrollbarDirectives.forEach((corpScrollbarDirective) =>
                    {
                        corpScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        ...
        setTimeout(() =>
        {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.corp-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._corpScrollbarDirectives.forEach((corpScrollbarDirective) =>
                {
                    // Skip if not enabled
                    if ( !corpScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    corpScrollbarDirective.scrollToElement('.corp-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: CorpNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: CorpNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('corp-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('corp-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('corp-vertical-navigation-aside-overlay');
